...
// copied from src/uucore/src/lib/mods/display.rs
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let cwd = if matches.contains_id(OPT_PHYSICAL) {
        physical_path()
    } else {
        logical_path()
    }
    ...
    println_verbatim(&cwd).expect("failed to print current directory");
